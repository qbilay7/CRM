<template>
    ...
    <div class="login-header" style="color: black;">Login as User</div>
    <form @submit.prevent="login">
      ...
      <div class="input-group">
        ...
        <input v-model="user.password" :type="showPassword ? 'text' : 'password'" id="password" name="password" required>
    ...
    methods:{
       async login(){
           try{
            ...
            this.isLoading=true;
                    await axios.post("https://api.golfsprint.net/golfsprint/api/auth/US/login",this.user,
                    { headers: {
                    ...
                    window.location.href = '/user/club';
                         })    
            }
            catch(error){
